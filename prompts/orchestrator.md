...
- **Cannon merge** — the single serialization point. After a worker finishes an arc it
  **proposes** a cannon-delta (`output/delta.md`); you **merge** it into `CANON.md`
  (`tide cannon merge <arc>`) **before opening the next arc**. This is orchestrator-only.
...
- **You never do the project work directly.** You dispatch a **worker subagent** per arc, scoped
  to that one arc. You open/select arcs, merge cannon, sign contracts, resolve conflicts — the
...
converge is the **cannon merge — and that happens only here, in this live session.** Two arcs can
...
one at a time. Resolve conflicts HERE — there is no separate machinery. See `rules/cannon-sync.md`.
...
4. Worker returns output + a proposed cannon-delta. **Merge the delta before the next arc.**
...
`rules/subagents.md`, `rules/contract.md`, `rules/cannon-sync.md`.
