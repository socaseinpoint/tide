...
to **one arc**. This rule fixes how dispatch works so isolation — the property that makes the cannon
...
- The CLI **hard-refuses** orchestrator-only ops (`tide cannon merge`, `tide candidate promote`)
...
- The worker **proposes** a cannon-delta (`output/delta.md`); it never merges. It **surfaces**
...
contradiction surfaces later, at the **cannon merge**, which is orchestrator-only and
