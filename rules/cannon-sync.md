# rule · cannon-sync (the serialization point)
...
## cannon-rev
- `cannon-rev` is a short content hash of **`CANON.md` only** (not the whole `cannon/` dir — so a
  note/changelog tweak never spams drift).
- It bumps on **every merge**.
- When an arc opens it **stamps** the current `cannon-rev` into its `arc.md` frontmatter.

## The flow
1. **Open** → stamp `cannon-rev`.
2. **Work** → the worker produces output + a **proposed** cannon-delta (`output/delta.md`).
3. **Merge (orchestrator-only)** → `tide cannon merge <arc>` folds the delta into `CANON.md`,
   appends a line to the **Cannon journal**, and **bumps `cannon-rev`**. Workers never merge.
4. **Before the next arc** → the orchestrator merges any owed delta first.

## Drift + the block (forced reconciliation)
- **Drift:** on dispatch/close, an open arc's stamped `cannon-rev` is compared to the current one.
  If the cannon moved underneath it → **flag drift**, force the worker to re-read `CANON.md` and
  re-stamp (`tide arc resume <arc>`) before its output is trusted.
- **Block:** opening a **new** arc is **refused** while a **closed** arc's cannon-delta is still
...
delta through this single point. ⇒ cannon is simultaneously the single source of truth and the only
